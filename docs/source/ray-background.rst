...
Its key innovations include an octree-accelerated method foe efficiently identifying grid-ray intersections, which reduces the time complexity of the search from :math:`{\cal O}(N^2)` to :math:`{\cal O}(N logN)`, and an analytical slab method for the rapid, precise calculation of penetration lengths.
